...
export const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case "DISMISS_TOAST": { const { toastId } = action; if (toastId) addToRemoveQueue(toastId); else state.toasts.forEach((t) => addToRemoveQueue(t.id)); return { ...state, toasts: state.toasts.map((t) => t.id === toastId || !toastId ? { ...t, open: false } : t) } }
    case "REMOVE_TOAST": return action.toastId ? { ...state, toasts: state.toasts.filter((t) => t.id !== action.toastId) } : { ...state, toasts: [] }
  }
}
