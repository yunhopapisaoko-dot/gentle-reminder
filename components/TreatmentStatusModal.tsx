...
export const TreatmentStatusModal: React.FC<TreatmentStatusModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[800] flex items-center justify-center p-6">
      ...
      {/* Card do Modal */}
      <div className={`relative w-full max-w-xs bg-background-dark border border-blue-500/30 rounded-[48px] overflow-hidden shadow-[0_0_80px_rgba(59,130,246,0.2)] ${isClosing ? 'animate-out zoom-out' : 'animate-in zoom-in duration-500'}`}>
        
        {/* Glow de fundo */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-40 h-40 bg-blue-500/20 blur-[60px] rounded-full pointer-events-none"></div>

        <div className="p-8 flex flex-col items-center">
          {/* Icone Animado */}
          <div className="relative mb-8">
            <div className="absolute -inset-4 bg-blue-500/20 blur-2xl rounded-full animate-pulse"></div>
            <div className="relative w-24 h-24 rounded-[32px] bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center text-white shadow-2xl border border-white/20">
               <span className="material-symbols-rounded text-5xl animate-[bounce_2s_infinite]">medical_services</span>
            </div>
          </div>

          <div className="text-center mb-8">
            <h3 className="text-xl font-black text-white italic uppercase tracking-tighter leading-none mb-2">Tratamento Ativo</h3>
            <p className="text-[10px] font-black text-blue-400 uppercase tracking-[0.3em]">Protocolo de Cura em curso</p>
          </div>
